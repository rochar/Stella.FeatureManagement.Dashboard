...
internal sealed class FeatureChangeValidation(
    ...
    ILogger<FeatureChangeValidation> logger)
    : IFeatureChangeValidation
{
    ...
    public FeatureChangeValidationResult CanProceed(FeatureFlagDto featureFlag, FeatureChangeType changeType)
    {
        ...
        {
            logger.LogDebug("Feature {FeatureName} is a registered managed feature, validating filters", featureFlag.Name);

            var registeredFilter = featureRegistration.GetFeatureRegistration(featureFlag.Name);
            if (registeredFilter is not null)
            {
                logger.LogDebug("Feature {FeatureName} has registered filter {FilterType}", featureFlag.Name, registeredFilter.TypeName);
                ...
                {
                    logger.LogWarning("Feature {FeatureName} is missing required filter {FilterType}", featureFlag.Name, registeredFilter.TypeName);
                ...
                try
                {
                    ...
                    logger.LogDebug("Deserializing filter parameters for feature {FeatureName} as {SettingsType}", featureFlag.Name, settingsType.Name);
                    ...
                    if (settings is null)
                    {
                        logger.LogWarning("Feature {FeatureName} filter parameters deserialized to null for {SettingsType}", featureFlag.Name, settingsType.Name);
                        return new FeatureChangeValidationResult(true,
                            $"{featureFlag.Name} filter parameters are not valid JSON for {settingsType.Name}.");
                    }

                    logger.LogDebug("Filter parameters validated successfully for feature {FeatureName}", featureFlag.Name);
                }
                catch (System.Text.Json.JsonException ex)
                {
                    logger.LogWarning(ex, "Feature {FeatureName} filter parameters are not valid JSON for {SettingsType}", featureFlag.Name, registeredFilter.DefaultSettings.GetType().Name);
                    return new FeatureChangeValidationResult(true,
        ...
        logger.LogDebug("Invoking custom feature change validator for feature {FeatureName}", featureFlag.Name);
        var result = featureChangeValidator(featureFlag, changeType);

        if (result.Cancel)
        {
            logger.LogInformation("Custom validator cancelled {ChangeType} for feature {FeatureName}: {CancellationMessage}", changeType, featureFlag.Name, result.CancellationMessage);
        }
        else
        {
            logger.LogDebug("Validation passed for {ChangeType} operation on feature {FeatureName}", changeType, featureFlag.Name);
        }
